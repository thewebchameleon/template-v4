...
public static class InvoicingEndpoints
{
    public static RouteGroupBuilder MapInvoicingEndpoints(this RouteGroupBuilder group)
    {
        ...
        invoices.MapGet("/settings", async (ClaimsPrincipal user, IInvoicing store, CancellationToken ct) => (await store.Settings(Actor(user), ct)).ToHttp()).WithName("GetInvoicingSettings").Produces<IssuerSettings>().ContinuesWhenDisabled(ModuleIds.Invoicing, "Read issuer details associated with retained obligations");
        invoices.MapGet("", async (ClaimsPrincipal user, IInvoicing store, CancellationToken ct, int pageNumber = 1, int pageSize = 10, string search = "", string sort = "issuedAt", string direction = "desc") => (await store.List(Actor(user), pageNumber, pageSize, search, sort, direction, ct)).ToHttp()).WithName("ListCommercialDocuments").Produces<Page<CommercialDocument>>().ContinuesWhenDisabled(ModuleIds.Invoicing, "Find retained commercial documents");
        invoices.MapGet("/{id:guid}", async (Guid id, ClaimsPrincipal user, IInvoicing store, CancellationToken ct) => (await store.Read(Actor(user), id, ct)).ToHttp()).WithName("GetCommercialDocument").Produces<CommercialDetail>().ContinuesWhenDisabled(ModuleIds.Invoicing, "Read retained commercial documents and correction history");
        invoices.MapPost("/{id:guid}/payment", async (Guid id, ClaimsPrincipal user, CommercialAction request, IInvoicing store, CancellationToken ct) => (await store.Settle(Actor(user), id, request, ct)).ToHttp()).WithName("RecordCommercialPayment").Produces<FinancialEntry>().ContinuesWhenDisabled(ModuleIds.Invoicing, "Settle an existing invoice after application permission and balance checks");
