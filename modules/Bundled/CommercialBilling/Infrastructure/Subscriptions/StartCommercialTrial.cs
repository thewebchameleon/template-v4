...
public sealed partial class CommercialBillingStore
{
    public async Task<Result<Unit>> Trial(Guid actor, StartCommercialTrial request, CancellationToken ct)
    {
        ...
        await using var tx = await db.Session.BeginTransactionAsync(ct); await TemplateV4.Infrastructure.Modules.ModuleActivationStore.LockRows(db, ct); await customers.Lock(ct);
