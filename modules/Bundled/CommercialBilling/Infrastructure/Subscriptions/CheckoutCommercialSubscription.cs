...
public sealed partial class CommercialBillingStore
{
    public async Task<Result<PaymentCheckout>> Checkout(Guid actor, CommercialCheckoutRequest request, CancellationToken ct)
    {
        ...
        {
            await TemplateV4.Infrastructure.Modules.ModuleActivationStore.LockRows(db, ct); await customers.Lock(ct); var account = await customers.Find(actor, ct); var methods = await paymentMethods.Status(ct);
