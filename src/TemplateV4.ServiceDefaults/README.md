...
.NET 10 ASP.NET Core defaults for health endpoints, readable multiline development logs, production JSON logs, OpenTelemetry logs, traces and metrics, service discovery and resilient outbound HTTP. Call builder.AddServiceDefaults() and app.MapDefaultEndpoints(). Unsafe HTTP methods are not automatically retried. Configure OTEL_EXPORTER_OTLP_ENDPOINT for your collector; protect health/monitoring endpoints through deployment topology.
