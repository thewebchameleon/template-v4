...
public sealed record CreateOrganisation(string Name);
...
public interface ICustomers : ICustomerAccess
{
    ...
    Task<Result<CustomerHome>> Administration(Guid actor, CancellationToken ct);
    Task<Result<Unit>> Select(Guid actor, Guid organisation, CancellationToken ct);
    Task<Result<CustomerInfo>> Create(Guid actor, CreateOrganisation request, CancellationToken ct);
    Task<Result<Unit>> Close(Guid actor, Guid customer, Guid version, CancellationToken ct);
    Task<Result<Unit>> Rename(Guid actor, Guid customer, RenameOrganisation request, CancellationToken ct);
