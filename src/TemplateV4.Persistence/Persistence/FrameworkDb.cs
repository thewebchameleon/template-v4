...
public class FrameworkDb : IdentityDbContext<AppUser, IdentityRole<Guid>, Guid>
{
    public DatabaseSession Session { get; }
    public FrameworkDb(DbContextOptions<FrameworkDb> options, DatabaseSession? session = null) : this((DbContextOptions)options, session ?? new DatabaseSession()) { }
    protected FrameworkDb(DbContextOptions options, DatabaseSession session) : base(options)
    {
        Session = session;
        session.Register(this, ct => base.SaveChangesAsync(ct));
    }
    public override Task<int> SaveChangesAsync(CancellationToken cancellationToken = default) => Session.SaveChangesAsync(cancellationToken);
    ...
    protected override void OnModelCreating(ModelBuilder model)
    {
        ...
        model.Entity<RuntimeModuleSettings>(entity =>
        {
            ...
            entity.Property(x => x.Version).IsConcurrencyToken();
            entity.HasData(new RuntimeModuleSettings { Id = "commercial-billing", Enabled = true, Version = new Guid("174a5cd6-5675-4ad7-a2e0-1cf14b7502fe") });
