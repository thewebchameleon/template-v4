...
async function library(page: Page, moves: { id: string; parentId: string | null }[] = []) {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    const move = path.match(/^\/api\/v1\/auth\/my-files\/([^/]+)\/move$/);
    ...
    if (path === `/api/v1/auth/my-files/${document.id}/shares`) {
    ...
    if (path === `/api/v1/auth/my-files/${document.id}/metadata`)
      return route.fulfill({ status: 204 });
    if (path === `/api/v1/auth/my-files/${document.id}/download`)
...
test('My Files keeps the parent item first and accepts list and grid drops', async ({ page }) => {
  ...
  const cards = page.locator('.my-files-grid > .my-files-grid-card');
  await expect(
...
test('My Files keeps compact card spacing and list-aligned grid sort headings', async ({
  ...
  const cardContent = page.locator('[data-slot="card-content"]').filter({
    has: page.locator('.my-files-controls'),
  });
  ...
  await page.getByRole('button', { name: 'Grid', exact: true }).click();
  const grid = page.locator('.my-files-grid');
  ...
  const gridStart = await page
    .locator('.my-files-grid-sort button')
    .first()
    .evaluate((element) => {
      const range = document.createRange();
      range.selectNodeContents(element);
...
for (const width of [390, 1440])
  for (const dark of [false, true]) {
    test(`My Files navigation, quota and recent cards at ${width}px ${dark ? 'dark' : 'light'}`, async ({
      ...
      await expect(recent.locator('.my-files-recent-card')).toHaveCount(6);
      ...
      await page.screenshot({
        path: `test-results/my-files-${width}-${dark ? 'dark' : 'light'}.png`,
        fullPage: true,
      });
      await page.goto('/my-files?folder=folder-a');
...
test('file details save inline and sharing opens an add-only dialog', async ({ page }) => {
  ...
  const downloadRequest = page.waitForRequest((request) =>
    request.url().endsWith(`/my-files/${document.id}/download`),
  );
  ...
  const metadataRequest = page.waitForRequest((request) =>
    request.url().endsWith(`/my-files/${document.id}/metadata`),
  );
  ...
  const revokeRequest = page.waitForRequest((request) =>
    request.url().endsWith(`/my-files/${document.id}/shares/share-a/revoke`),
  );
  await drawer
