import { Directive } from '@angular/core';
import { BrnPopoverContent } from '@spartan-ng/brain/popover';
import * as i0 from "@angular/core";
import * as i1 from "@spartan-ng/brain/popover";
export class HlmSelectPortal {
    static ɵfac = function HlmSelectPortal_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || HlmSelectPortal)(); };
    static ɵdir = /*@__PURE__*/ i0.ɵɵdefineDirective({ type: HlmSelectPortal, selectors: [["", "hlmSelectPortal", ""]], features: [i0.ɵɵHostDirectivesFeature([{ directive: i1.BrnPopoverContent, inputs: ["context", "context", "class", "class"] }])] });
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(HlmSelectPortal, [{
        type: Directive,
        args: [{
                selector: '[hlmSelectPortal]',
                hostDirectives: [{ directive: BrnPopoverContent, inputs: ['context', 'class'] }],
            }]
    }], null, null); })();
