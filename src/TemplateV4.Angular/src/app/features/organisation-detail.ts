...
@Component({
  ...
  template: `<app-page-header title="organisationWorkspace" description="organisationWorkspaceHelp"
      ><a hlmBtn variant="outline" routerLink="/administration/organisations">{{
        'organisations' | t
      }}</a>
    </app-page-header>
    <app-page-state [state]="home.state()" [refreshError]="home.refreshError()" (retry)="reload()">
      @if (account(); as account) {
        ...
        <div class="flex flex-wrap gap-2 mb-6">
          <button hlmBtn variant="destructive" [disabled]="busy()" (click)="close()">
            {{ 'closeOrganisation' | t }}
          </button>
        </div>
        <section hlmCard class="mb-6">
          <div hlmCardHeader>
            <h2 hlmCardTitle>{{ 'organisationMembers' | t }}</h2>
        ...
        @if (auth.access()?.isAdministrator) {
          ...
          <section hlmCard>
            ...
            <form hlmCardContent class="grid gap-4" (ngSubmit)="rename()" #renameForm="ngForm">
              <div hlmField>
                <label hlmFieldLabel for="rename-organisation">{{ 'organisationName' | t }}</label
                ><input
                  hlmInput
                  id="rename-organisation"
                  name="name"
                  [(ngModel)]="name"
                  required
                  maxlength="120"
...
export class OrganisationDetailPage {
  ...
  readonly columns = computed(() => {
    this.i18n.culture();
    const owner = this.auth.access()?.isAdministrator === true;
  ...
  async reload(preserveDraft = true) {
    ...
    if (await this.home.load((signal) => this.api.get('customers/administration', {}, signal)))
  ...
  async close() {
    ...
    if (!(await this.confirm.ask('closeOrganisation', 'closeOrganisationHelp', account.name, true)))
      return;
    this.busy.set(true);
    try {
      ...
      this.busy.set(false);
      await this.router.navigateByUrl('/administration/organisations');
    } finally {
      this.busy.set(false);
    }
  }
