...
export async function connectNotifications(options) {
    const accessToken = async () => {
        const access = options.access();
        if (!access || access.userId !== options.actor)
            return '';
    ...
    const connection = new HubConnectionBuilder()
        ...
        nextRetryDelayInMilliseconds: (context) => Math.min(1000 * 2 ** context.previousRetryCount, 60000),
    })
        .build();
