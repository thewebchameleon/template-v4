export function filterVisibleFileGroups(groups) {
    return groups.filter(() => true);
}
export function filterVisibleFileFolders(group, folders) {
    return group === 'my-files'
        ? folders.filter(() => true)
...
export function canMoveFolder(source, parentId, folders) {
    ...
    while (parent !== null) {
        ...
        if (!folder?.isFolder || folder.permission !== 'owner')
            return false;
        parent = folder.parentId;
    }
    return true;
}
export function canMoveEntry(source, parentId, folders) {
    if (source.permission !== 'owner' || source.parentId === parentId)
        return false;
    if (source.isFolder)
        return canMoveFolder(source, parentId, folders);
    if (parentId === null)
        return true;
