...
export type MyFilesActionMode = 'create' | 'move' | '';
export interface MyFilesMoveDestination {
  value: string;
  label: string;
  disabled?: boolean;
}
...
export class MyFilesActionDialog {
  readonly mode = input<MyFilesActionMode>('');
  readonly description = input('');
  readonly destinations = input<readonly MyFilesMoveDestination[]>([]);
  readonly busy = input(false);
  readonly fieldId = input('my-files-action');
...
export class MyFilesTree {
  ...
  private readonly navigation = inject(MyFilesNavigation);
  ...
  readonly selectedGroup = signal('my-files');
  ...
  async dropFolder(event: DragEvent, parent: string | null, group: string) {
    ...
    const source = this.dragged();
    this.endDrag();
    if (group === 'trash' || (!parent && group !== 'my-files') || !source) return;
    await this.moveFolder(source, parent);
  }
  contextMoveDestinations(): MyFilesMoveDestination[] {
  ...
  private async moveFolder(source: FileItem, parentId: string | null) {
    if (this.busy() || !canMoveFolder(source, parentId, this.folders('my-files'))) return;
    this.busy.set(true);
    try {
      ...
      Object.fromEntries(
        await Promise.all(
          this.groups.map(async (group) => [
            group.id,
            await this.api.get<FilePage>('my-files', { group: group.id, pageSize: 1 }, signal),
          ]),
        ),
      ),
    );
