...
import { OrganisationFileItem, OrganisationFilePage } from '../../../api/models';
...
const column = createColumnHelper<DataTableFeatures, OrganisationFileItem>();
@Component({
...
export class OrganisationFilesPage {
  ...
  readonly data = new Resource<OrganisationFilePage>();
  ...
  async remove(file: OrganisationFileItem) {
    if (
      this.busy() ||
