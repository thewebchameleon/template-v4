...
export class GradientQuality {
    ...
    dimensions(width, height, pixelRatio) {
        ...
        const ratio = Math.min(pixelRatio || 1, (edge * this.scale) / Math.max(width, height), Math.sqrt((pixels * this.scale * this.scale) / (width * height)));
    ...
    record(milliseconds) {
        ...
        if (milliseconds < 33 && this.samples < 8)
            return;
        ...
        if (average > 12) {
            ...
            this.scale = Math.max(this.expensive ? 0.4 : 0.75, this.scale * Math.max(0.5, Math.sqrt(10 / average)));
        }
        this.samples = 0;
        this.cost = 0;
    }
