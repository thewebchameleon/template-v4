import { LoginBackground } from '../core/login-backgrounds';
import {
  paintGradient,
  defaultsByType,
  presetsByType,
...
export class GradientRenderer {
  ...
  render(preset: LoginBackground, time: number, width: number, height: number) {
    ...
    paintGradient(
      ...
      p['divs'],
      p['mesh'],
      p['flow'] ?? p['stripe'] ?? p['bars'] ?? p['cnoise'] ?? p['ring'] ?? p['smesh'] ?? p['silk'],
      time,
      p['scale'],
      p['hexStyle'],
      p['ballStyle'],
