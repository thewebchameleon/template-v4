...
export const validPrimaryColor = (value) => /^#[0-9a-f]{6}$/i.test(value);
...
function shade(hex, targetLuminance) {
    ...
    for (let step = 0; step < 20; step++) {
        ...
        if (target === 255 ? value < targetLuminance : value > targetLuminance)
            low = middle;
        else
            high = middle;
    }
