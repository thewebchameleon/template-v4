...
export function recolorBackground(preset: LoginBackground, primaryColor: string): LoginBackground {
  ...
  const anchor = colors.reduce((best, color) =>
    color.s * color.v > best.s * best.v ? color : best,
  );
  return {
    ...preset,
...
export function loginBackground(id: unknown): LoginBackground {
  return (
    LOGIN_BACKGROUNDS.find((preset) => preset.id === id) ??
