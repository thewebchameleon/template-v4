...
public static class CustomerBillingEndpoints
{
    ...
    public static RouteGroupBuilder MapCustomerBillingEndpoints(this RouteGroupBuilder group)
    {
        ...
        organisationEntries.MapGet("/", async (ClaimsPrincipal u, ICustomers store, CancellationToken ct) => (await store.Home(Actor(u), ct)).ToHttp()).WithName("GetCustomers").Produces<CustomerHome>();
        ...
        organisationEntries.MapPost("/current", async (ClaimsPrincipal u, SelectOrganisation r, ICustomers store, CancellationToken ct) => (await store.Select(Actor(u), r.OrganisationId, ct)).ToHttp()).WithName("SelectOrganisation");
