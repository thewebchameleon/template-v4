...
public static class FileStorageEndpoints
{
    public static RouteGroupBuilder MapFileStorageEndpoints(this RouteGroupBuilder group)
    {
        ...
        files.MapPost("/batch/delete", async (FileSelectionRequest request, ClaimsPrincipal principal, FileStorageService service, CancellationToken ct) => (await service.DeleteBatch(EndpointSecurity.Actor(principal), request, ct)).ToHttp()).RequireAuthorization().WithName("DeleteFileStorageBatch");
        files.MapPost("/batch/move", async (FileBatchDestinationRequest request, ClaimsPrincipal principal, FileStorageService service, CancellationToken ct) => (await service.MoveBatch(EndpointSecurity.Actor(principal), request, ct)).ToHttp()).RequireAuthorization().WithName("MoveFileStorageBatch");
        files.MapPost("/batch/copy", async (FileBatchDestinationRequest request, ClaimsPrincipal principal, FileStorageService service, CancellationToken ct) => (await service.CopyBatch(EndpointSecurity.Actor(principal), request, ct)).ToHttp()).RequireAuthorization().WithName("CopyFileStorageBatch");
        files.MapPost("/batch/download", async (FileSelectionRequest request, ClaimsPrincipal principal, FileStorageService service, CancellationToken ct) =>
        {
            var result = await service.DownloadBatch(EndpointSecurity.Actor(principal), request, ct);
            return result.IsSuccess ? Results.File(result.Value!.Content, "application/zip", result.Value.Name, enableRangeProcessing: false) : ApiResults.Failure(result.Error!);
        }).RequireAuthorization().WithName("DownloadFileStorageBatch").Produces(200, contentType: "application/zip");
        ...
        files.MapPost("/trash/empty", async (ClaimsPrincipal principal, FileStorageService service, CancellationToken ct) => (await service.EmptyTrash(EndpointSecurity.Actor(principal), ct)).ToHttp()).RequireAuthorization().WithName("EmptyFileStorageTrash");
        ...
        files.MapGet("/public/{id:guid}", async (Guid id, HttpContext context, FileStorageService service, [FromHeader(Name = "X-File-Share")] string? shareToken, CancellationToken ct) => (await service.PublicItem(id, ShareToken(context, shareToken), ct)).ToHttp()).AllowAnonymous().WithName("GetPublicFileStorageFile").Produces<PublicFileShare>();
