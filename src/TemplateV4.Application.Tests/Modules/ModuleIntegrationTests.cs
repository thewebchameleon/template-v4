...
public sealed partial class SecurityAndMessagingTests
{
    ...
    public async Task Disabled_modules_deny_an_authorized_actor_without_erasing_audit_data()
    {
        ...
        // A selected client's private modules can explicitly enable dependencies of the minimal preset.
        await using var baseline = new ApiFactory(_configuration);
        foreach (var module in baseline.Services.GetRequiredService<TemplateV4.Application.Modules.ModuleCatalog>().Definitions.Where(module => module.Category == "private"))
            configuration[$"Modules:{module.Id}"] = "false";
