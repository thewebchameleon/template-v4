...
public static class CrmMappings
{
    public static void Configure(ModelBuilder model)
    {
        ...
        records.ToTable("records", "crm"); records.HasKey(x => new { x.OrganisationId, x.Id });
        ...
        records.HasIndex(x => new { x.OrganisationId, x.Kind, x.Archived, x.Name, x.Id });
        var configuration = model.Entity<CrmConfigurationRow>(); configuration.ToTable("configuration", "crm");
        configuration.HasKey(x => x.OrganisationId); configuration.Property(x => x.Version).IsConcurrencyToken();
        configuration.Property(x => x.Data).HasColumnType("jsonb");
        var notes = model.Entity<CrmNoteRow>(); notes.ToTable("notes", "crm"); notes.HasKey(x => new { x.OrganisationId, x.Id });
        notes.Property(x => x.Text).HasMaxLength(8000);
        notes.HasOne<CrmRecordRow>().WithMany().HasForeignKey(x => new { x.OrganisationId, x.RecordId }).OnDelete(DeleteBehavior.Restrict);
        notes.HasIndex(x => new { x.OrganisationId, x.RecordId, x.At });
    }
}
