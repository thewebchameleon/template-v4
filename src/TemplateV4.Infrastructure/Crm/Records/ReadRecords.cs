...
{
    public async Task<Result<CrmRecord>> Resolve(Guid actor, Guid id, bool allowArchived, CancellationToken ct)
    {
        if (!await access.Allowed(actor, OrganisationOperation.Read, ct)) return Result<CrmRecord>.Fail("customers.not_found", ErrorKind.NotFound);
        var row = await Records().AsNoTracking().SingleOrDefaultAsync(x => x.Id == id && (allowArchived || !x.Archived) && x.Kind != "Deal", ct);
        return row is null ? Result<CrmRecord>.Fail("resource.not_found", ErrorKind.NotFound) : Result<CrmRecord>.Success(Read(row));
    }
    public async Task<Result<Page<CrmRecord>>> List(Guid actor, CrmList query, CancellationToken ct)
    {
        ...
        if (!await access.Allowed(actor, OrganisationOperation.Read, ct)) return Result<Page<CrmRecord>>.Fail("customers.not_found", ErrorKind.NotFound);
        var kind = query.Kind.ToString(); var search = query.Search.Trim().ToLowerInvariant();
        var rows = Records().AsNoTracking().Where(x => x.Kind == kind && x.Archived == query.Archived);
    ...
    public async Task<Result<CrmDetail>> Detail(Guid actor, Guid id, CancellationToken ct)
    {
        if (!await access.Allowed(actor, OrganisationOperation.Read, ct)) return Result<CrmDetail>.Fail("customers.not_found", ErrorKind.NotFound);
        var row = await Records().AsNoTracking().SingleOrDefaultAsync(x => x.Id == id, ct);
        if (row is null) return Result<CrmDetail>.Fail("resource.not_found", ErrorKind.NotFound);
        var notes = await db.Set<CrmNoteRow>().AsNoTracking().Where(x => x.RecordId == id)
