...
public sealed partial class CrmStore
{
    ...
    private async Task<CrmConfigurationRow> Seed(CancellationToken ct)
    {
        var row = await db.Set<CrmConfigurationRow>().SingleOrDefaultAsync(ct);
        ...
        row = new() { Version = configuration.Version, Data = JsonSerializer.Serialize(configuration, Json) };
        db.Set<CrmConfigurationRow>().Add(row); return row;
    }
    public async Task<Result<CrmConfiguration>> Configuration(Guid actor, CancellationToken ct)
    {
        await using var tx = await db.Database.BeginTransactionAsync(ct); await Lock(ct);
        if (!await access.Allowed(actor, OrganisationOperation.Read, ct)) return Result<CrmConfiguration>.Fail("customers.not_found", ErrorKind.NotFound);
        var row = await Seed(ct); await db.SaveChangesAsync(ct); await tx.CommitAsync(ct); return Result<CrmConfiguration>.Success(ReadConfiguration(row));
    }
    public async Task<Result<CrmConfiguration>> Configure(Guid actor, CrmConfiguration request, CancellationToken ct)
    {
        ...
        await using var tx = await db.Database.BeginTransactionAsync(ct); await Lock(ct);
        if (!await access.Allowed(actor, OrganisationOperation.Configure, ct)) return Result<CrmConfiguration>.Fail("authorization.denied", ErrorKind.Forbidden);
        var row = await Seed(ct);
        ...
        Audit(actor, Organisation.Id, "crm.configuration_changed");
