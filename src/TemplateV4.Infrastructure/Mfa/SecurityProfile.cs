...
public sealed partial class SecurityService
{
    public async Task<ProfileResponse> Profile(Guid id, CancellationToken ct)
    {
        ...
        return new(id, user.Email!, user.UserName!, profile.DisplayName, profile.Culture, (await users.GetRolesAsync(user)).ToArray(), user.TwoFactorEnabled, await GloballyRequired(user, ct), await users.CountRecoveryCodesAsync(user),
