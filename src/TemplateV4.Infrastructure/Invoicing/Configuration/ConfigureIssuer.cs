...
{
    public async Task<Result<IssuerSettings>> Settings(Guid actor, CancellationToken ct)
    {
        if (!await access.Allowed(actor, OrganisationOperation.Read, ct)) return Result<IssuerSettings>.Fail("customers.not_found", ErrorKind.NotFound);
        var row = await db.Set<IssuerSettingsRow>().AsNoTracking().SingleOrDefaultAsync(ct);
        return Result<IssuerSettings>.Success(row is null ? new(Guid.Empty, "", "", "", "", false, null, "DOC") : Read(row));
    }
    public async Task<Result<IssuerSettings>> Configure(Guid actor, IssuerSettings request, CancellationToken ct)
    {
        ...
        if (!await access.Allowed(actor, OrganisationOperation.Configure, ct)) return Result<IssuerSettings>.Fail("authorization.denied", ErrorKind.Forbidden);
        var row = await db.Set<IssuerSettingsRow>().SingleOrDefaultAsync(ct);
        if ((row?.Version ?? Guid.Empty) != request.Version) return Result<IssuerSettings>.Fail("concurrency.conflict", ErrorKind.Conflict);
        if (row is null) { row = new() { Id = 1 }; db.Set<IssuerSettingsRow>().Add(row); }
        row.Version = Guid.NewGuid(); row.Data = JsonSerializer.Serialize(request with { Version = row.Version }, Json);
        Audit(actor, Organisation.Id, "invoicing.settings_changed"); await db.SaveChangesAsync(ct); await tx.CommitAsync(ct);
