...
{
    public async Task<Result<CommercialDocument>> Issue(Guid actor, IssueCommercialDocument request, CancellationToken ct)
    {
        ...
        if (retry != null) return retry.Fingerprint == hash ? Result<CommercialDocument>.Success(Read(await Documents().SingleAsync(x => x.Id == retry.ResultId, ct))) : Result<CommercialDocument>.Fail("idempotency.conflict", ErrorKind.Conflict);
        if (request.Kind == CommercialDocumentKind.Invoice && request.Origin is { } origin)
        {
            var existing = await Documents().SingleOrDefaultAsync(x => x.Kind == "Invoice" && x.OriginModule == origin.Module && x.OriginType == origin.Type && x.OriginId == origin.Id, ct);
            if (existing != null) return Result<CommercialDocument>.Success(Read(existing));
        }
        var settings = await db.Set<IssuerSettingsRow>().SingleOrDefaultAsync(ct);
        ...
        {
            var quote = await Documents().SingleOrDefaultAsync(x => x.Id == quoteId && x.Kind == "Quotation" && x.Accepted, ct);
            ...
            if (await Documents().AnyAsync(x => x.QuotationId == quoteId && x.Kind == "Invoice", ct)) return Result<CommercialDocument>.Fail("invoicing.quotation_invoiced", ErrorKind.Conflict);
        ...
        {
            var customer = await crm.Resolve(actor, request.CustomerId, false, ct);
        ...
        {
            var previous = await Documents().SingleOrDefaultAsync(x => x.Id == previousId && x.Kind == "Quotation", ct);
            if (previous is null || previous.Accepted || previous.CustomerId != request.CustomerId || Read(previous).Origin != request.Origin ||
                await Documents().AnyAsync(x => x.PreviousRevisionId == previousId, ct)) return Result<CommercialDocument>.Fail("invoicing.revision_conflict", ErrorKind.Conflict);
        }
