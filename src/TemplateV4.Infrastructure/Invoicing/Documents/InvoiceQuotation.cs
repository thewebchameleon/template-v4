...
{
    public async Task<Result<CommercialDocument>> InvoiceAccepted(Guid actor, Guid quotation, Guid idempotencyKey, CancellationToken ct)
    {
        var detail = await Read(actor, quotation, ct);
        ...
        return await Issue(actor, new(idempotencyKey, quote.CustomerId, CommercialDocumentKind.Invoice,
