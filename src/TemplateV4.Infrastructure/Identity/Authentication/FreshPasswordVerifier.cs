...
/// <summary>Fresh password proof for destructive operations. Attempts commit independently of the caller's transaction.</summary>
public sealed class FreshPasswordVerifier(FrameworkDb db, IConfiguration configuration, TimeProvider time, IPasswordHasher<AppUser> passwords)
{
    public async Task<bool> Verify(Guid actor, string purpose, string? password, CancellationToken ct)
    {
        if (string.IsNullOrEmpty(password) || password.Length > 1024) return false;
        var now = time.GetUtcNow();
        var bucket = AuthService.Hash($"fresh-password:{purpose}:{actor}:{now.ToUnixTimeSeconds() / 900}");
