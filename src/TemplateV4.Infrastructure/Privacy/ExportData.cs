...
public sealed partial class PrivacyService
{
    public async Task<byte[]> Export(Guid actor, CancellationToken ct)
    {
        ...
        var fileShares = await db.Set<FileStorageShare>().AsNoTracking().Where(x => x.RecipientId == actor || x.SharedById == actor || db.Files.Any(f => f.Id == x.FileId && f.OwnerId == actor)).Select(x => new { x.FileId, x.SharedById, x.RecipientId, x.ExpiresAt }).ToArrayAsync(ct);
