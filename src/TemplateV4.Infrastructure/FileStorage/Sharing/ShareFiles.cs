...
public sealed partial class FileStorageService
{
    public async Task<Result<FileShareItem[]>> Shares(Guid actor, Guid id, CancellationToken ct)
    {
        ...
        return Result<FileShareItem[]>.Success(await db.Set<FileStorageShare>().Where(x => x.FileId == id).OrderByDescending(x => x.CreatedAt).Select(x => new FileShareItem(x.Id, db.Users.Where(u => u.Id == x.RecipientId).Select(u => u.Email).FirstOrDefault(), x.Permission, x.ExpiresAt, null)).ToArrayAsync(ct));
    }
    public async Task<Result<FileShareItem>> Share(Guid actor, Guid id, FileShareRequest request, CancellationToken ct)
    {
        ...
        var row = new FileStorageShare { FileId = id, RecipientId = recipient, Permission = request.Permission, ExpiresAt = request.ExpiresAt?.ToUniversalTime(), CreatedAt = time.GetUtcNow(), TokenHash = token is null ? null : Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(token))) };
