...
public sealed partial class FileStorageService
{
    ...
    private async Task<string?> Permission(Guid actor, StoredFile file, CancellationToken ct, string? token = null)
    {
        ...
        var shares = db.Set<FileStorageShare>().Where(x => ancestors.Contains(x.FileId) && x.RevokedAt == null && (x.ExpiresAt == null || x.ExpiresAt > now));
        if (token != null)
        {
    ...
    private async Task<StoredFile?> Access(Guid actor, Guid id, bool edit, CancellationToken ct, string? token = null)
    {
        ...
        return permission != null && (!edit || permission == "owner") ? file : null;
    }
...
public sealed record PublicFileShare(FileItem File, string? SharedByDisplayName, string? SharedByEmail, bool Revoked);
