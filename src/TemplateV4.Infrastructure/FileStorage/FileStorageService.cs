    ...
    IStorageUsage storageUsage, IStorageQuota storageQuota)
{
    ...
    private Task<bool> FolderNameExists(Guid? parent, string name, Guid? excludedId, CancellationToken ct)
    {
        var normalized = name.Trim().ToLowerInvariant();
        return db.Files.AnyAsync(x => x.Id != excludedId && x.ParentId == parent && x.IsFolder && x.Ready && x.DeletedAt == null && x.PurgedAt == null && x.Name.ToLower() == normalized, ct);
    }
