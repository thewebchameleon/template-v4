    ...
    >
      <section hlmCard class="min-w-0">
        ...
        <div hlmCardContent class="flex min-w-0 flex-col gap-5">
          ...
          <div class="relative min-w-0 max-w-full overflow-x-auto">
            <table hlmTable>
              <thead hlmTHead>
                <tr hlmTr>
                  <th hlmTh>{{ 'name' | t }}</th>
                  <th hlmTh>{{ 'role' | t }}</th>
                  <th hlmTh>{{ 'status' | t }}</th>
                  <th hlmTh>{{ 'actions' | t }}</th>
                </tr>
              </thead>
              <tbody hlmTBody>
                @for (user of page().items; track user.id) {
                  <tr hlmTr>
                    ...
                    <td hlmTd>
                      @if (auth.has('users.manage') && user.id !== auth.access()?.userId) {
                        @if (user.status === 'Invited') {
                          <button
                            hlmBtn
                            variant="outline"
                            [disabled]="busy()"
                            (click)="invitation(user, false)"
                          >
                            {{ 'resendInvitation' | t }}
                          </button>
                          <button
                        ...
                        <button
                          hlmBtn
                          variant="outline"
                          [disabled]="busy() || !roleDrafts.has(user.id)"
