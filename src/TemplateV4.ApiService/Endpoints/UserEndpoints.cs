...
public static class UserEndpoints
{
    ...
    {
        group.MapGet("/users", async (Dispatcher<ListUsers, Page<UserDto>> dispatcher, CancellationToken ct, int pageNumber = 1, int pageSize = 25, string? search = null, string sort = "name") =>
                (await dispatcher.Send(new(pageNumber, pageSize, search, sort), ct)).ToHttp())
