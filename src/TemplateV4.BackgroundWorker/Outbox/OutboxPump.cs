...
public sealed class OutboxPump(IServiceScopeFactory scopes, ILogger<OutboxPump> logger) : BackgroundService
{
    ...
    public async Task<bool> Process(CancellationToken ct)
    {
        ...
        try
        {
            ...
            await using var local = message.Type is "email.requested.v1" or "push.requested.v1" or "contact.notification.v1" or "invoicing.pdf-email-requested.v1" ? null : await db.Session.BeginTransactionAsync(ct);
