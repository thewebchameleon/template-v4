...
export function backend(modules, host) {
  const api = host === "TemplateV4.ApiService";
  const array = (values) => `[${values.map(literal).join(", ")}]`;
  const descriptors = modules.map(
    (d) =>
      `new(${literal(d.id)}, false, true, ${array(d.dependencies)}, true, ${literal(d.featureFlag ?? null)}, [${(d.capabilities ?? []).map((c) => `new(${literal(c.id)}, ${array(c.requires)}, ${literal(c.featureFlag ?? null)})`).join(", ")}])`,
  );
...
if (
  ...
  const content = host ? backend(modules, host) : frontend(modules);
  if (!host) {
