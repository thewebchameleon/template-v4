...
export function selectedIds(root) {
  const ids = readClientModules(root).privateModules;
  if (
...
export function discover(root, selection = selectedIds(root)) {
  const directory = path.join(root, "business-modules");
  const modules = [];
  for (const entry of fs.existsSync(directory)
    ...
    for (const layer of ["Api", "Infrastructure"]) {
      const files = fs
    ...
    if (!fs.existsSync(path.join(folder, "frontend/public-api.ts")))
...
export function backend(modules, host, foundation = {}, installed = null) {
  ...
  const descriptors = modules.map(
    (d) =>
      `new(${literal(d.id)}, false, true, ${array(d.dependencies)}, true, ${literal(d.featureFlag ?? null)}, [${(d.capabilities ?? []).map((c) => `new(${literal(c.id)}, ${array(c.requires)}, ${literal(c.featureFlag ?? null)})`).join(", ")}], Category: "private")`,
  );
  return `// <auto-generated />\nnamespace TemplateV4.Host;\ninternal static class BusinessModules\n{\n    internal static TemplateV4.Application.Modules.ModuleDefinition[] Descriptors => [${descriptors.join(", ")}];\n    internal static void ConfigureClient(Microsoft.AspNetCore.Builder.WebApplicationBuilder builder)\n    {\n${Object.entries(
    foundation,
  )
    .map(
      ([id, enabled]) =>
...
if (
  ...
  if (
    ...
    checkGeneratedSelection(
      repository,
      readClientModules(repository).privateModules,
    );
  ...
  if (!host && !process.argv.includes("--check"))
    fs.writeFileSync(
      ...
      selectionText(readClientModules(repository).privateModules),
    );
  if (!host) {
