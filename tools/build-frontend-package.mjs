...
function rewrite(folder){for(const entry of fs.readdirSync(folder,{withFileTypes:true})){
 ...
 else if(entry.name.endsWith('.ts')){const source=fs.readFileSync(target,'utf8');fs.writeFileSync(target,source.replace(/(['"])(@spartan-ng\/helm\/[^'"]+)\1/g,(match,quote,id)=>{
...
rewrite(path.join(staging,'src'));rewrite(path.join(staging,'libs'));
