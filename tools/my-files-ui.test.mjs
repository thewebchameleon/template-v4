...
const source = readFileSync(
  new URL(
    "../src/TemplateV4.Angular/src/app/features/my-files/files/my-files-ui.ts",
    import.meta.url,
  ),
  "utf8",
);
...
test("submenu keeps every file group visible regardless of its file count", () => {
  ...
  assert.deepEqual(
    ...
    ["my-files", "important", "shared", "recent", "starred", "trash"],
  );
});
...
test("group folders require direct files rather than nested folders", () => {
  ...
  assert.deepEqual(
    filterVisibleFileFolders("my-files", folders).map((entry) => entry.id),
