  ...
  case 'new': {
    ...
    if (kind === 'feature' || kind === 'module') {
      ...
      const files = [
        ...
        [`${folder}/README.md`, `# ${name}\n\nThis read-only starter returns its feature name. Replace that behavior with your Application contract.\n\nRegister IHandler<${name}Query, string> with ${name}Handler in the owner's Infrastructure/${owner}/${owner}Registration.cs. Add Permissions.${name}Read to the explicit Permissions.All catalog, call api.Map${name}(), and add the lazy Angular route. The host registers policies and the Migrator synchronizes built-in grants; delegated roles receive no automatic grants. Regenerate API clients, replace the page text with localisation keys, and add behavioral tests. For writes follow documentation/docs/user-management.md: explicit validator, transaction, domain event, outbox and BackgroundWorker consumer. Use the existing Support, CMS, CRM and Invoicing workflows from documentation/docs/module-ownership.md as references.\n`]
      ];
