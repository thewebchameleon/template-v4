...
switch (command) {
  ...
  case 'upgrade':
    console.log(`Installed framework ${manifest.frameworkVersion}. No automatic migrations are available for this initial release. Review documentation/docs/deployment.md before changing pins.`); break;
  case 'new': {
    ...
    const http = manifest.projects.Http ?? manifest.projects.API;
    ...
    if (kind === 'feature' || kind === 'module') {
      ...
      const files = [
        ...
        [`${http}/${owner}/${name}Endpoints.cs`, `using TemplateV4.Application;\nusing TemplateV4.Application.${name};\nusing TemplateV4.SharedKernel;\n\nnamespace TemplateV4.ApiService.Endpoints;\n\npublic static class ${name}Endpoints\n{\n    public static RouteGroupBuilder Map${name}(this RouteGroupBuilder group)\n    {\n        group.MapGet("/${name.toLowerCase()}", async (Dispatcher<${name}Query, string> dispatcher, CancellationToken ct) => (await dispatcher.Send(new(), ct)).ToHttp()).RequireAuthorization("${name.toLowerCase()}.read").WithName("Get${name}");\n        return group;\n    }\n}\n`],
        [`${manifest.projects.Web}/src/app/features/${featureOwner ?? name.toLowerCase()}/${name.toLowerCase()}.ts`, `import { Component } from '@angular/core';\nimport { WorkspaceUi } from '../../shared/workspace';\n@Component({selector:'app-${name.toLowerCase()}',imports:[WorkspaceUi],template:'<app-page-header title="${name.toLowerCase()}.title" description="${name.toLowerCase()}.description" /><section hlmCard><div hlmCardHeader><h2 hlmCardTitle>${name}</h2></div><div hlmCardContent></div></section>'})\nexport class ${name}Page {}\n`],
        [`${folder}/README.md`, `# ${name}\n\nThis read-only starter returns its feature name. Replace that behavior with your Application contract.\n\nRegister IHandler<${name}Query, string> with ${name}Handler in the owner's Infrastructure/${owner}/${owner}Registration.cs. Add Permissions.${name}Read to the explicit Permissions.All catalog, call api.Map${name}(), and add the lazy Angular route. The host registers policies and the Migrator synchronizes built-in grants; delegated roles receive no automatic grants. Regenerate API clients and replace the page text with localisation keys. For writes follow documentation/docs/architecture.md and documentation/docs/modules.md: use an explicit validator, transaction, domain event, outbox, and BackgroundWorker consumer. Use Support, CMS, CRM, and Invoicing as references.\n`]
      ];
      files.push([`${app}/${owner}/${name}Permissions.cs`, permissionSource]);
      files[0][1] = files[0][1].replace(`"${name.toLowerCase()}.read"`, `TemplateV4.Application.Users.Permissions.${name}Read`);
      files[1][1] = files[1][1].replace(`RequireAuthorization("${name.toLowerCase()}.read")`, `RequireAuthorization(TemplateV4.Application.Users.Permissions.${name}Read)`);
      if (kind === 'feature') {
      ...
      if (kind === 'module') {
        ...
        files.push(
          [`${manifest.projects.Infrastructure}/${name}/${name}Registration.cs`, `using Microsoft.Extensions.DependencyInjection;\nusing TemplateV4.Application.${name};\nusing TemplateV4.SharedKernel;\n\nnamespace TemplateV4.Infrastructure;\n\npublic static partial class Registration\n{\n    private static void Add${name}(IServiceCollection services)\n    {\n        services.AddScoped<IHandler<${name}Query, string>, ${name}Handler>();\n    }\n}\n`],
        ...
        files[3][1] += `\nCall Add${name}(services) from AddInfrastructure after reviewing the implementation. Keep registration unconditional so disabled modules retain approved recovery and cleanup dependencies.\n`;
      }
    ...
    const templates = {
      ...
      permission: [`${app}/${name}/${name}Permissions.cs`, permissionSource],
      ...
      endpoint: [`${http}/${name}/${name}Endpoints.cs`, `namespace TemplateV4.ApiService.Endpoints;\n\npublic static class ${name}Endpoints\n{\n    public static RouteGroupBuilder Map${name}(this RouteGroupBuilder group)\n    {\n        // Register explicit dispatcher-backed endpoints and permission policies here.\n        return group;\n    }\n}\n`],
