...
dotnet pack (Join-Path $repo 'src/TemplateV4.SharedKernel') -c Release -o $output
if ($LASTEXITCODE -ne 0) { throw 'Package build failed' }
New-Item -ItemType Directory -Force -Path $consumer | Out-Null
@'
<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><ManagePackageVersionsCentrally>false</ManagePackageVersionsCentrally><TargetFramework>net10.0</TargetFramework><OutputType>Exe</OutputType><ImplicitUsings>enable</ImplicitUsings></PropertyGroup><ItemGroup><PackageReference Include="TemplateV4.SharedKernel" Version="0.1.0" /></ItemGroup></Project>
