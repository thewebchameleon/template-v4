...
Administrators can open **Administration → Modules** to enable or disable File Storage for the whole application. Change **Enable File Storage**, then **Save**. Disabling hides File Storage and its settings, and blocks library and administrative storage APIs. Existing files and quotas are preserved; normal retention and privacy processing continue. Re-enable File Storage to restore access. Other open browsers update navigation on their next navigation or reload; the API enforces changes on subsequent requests immediately.
...
Signed-in users land on **Dashboard** after completing required security setup. Administrators open **Administration** (the settings-cog icon) from the navigation rail, then **User Management**. The workspace has bookmarkable **Users** (`/administration/users`), **Invitations** (`/administration/users/invitations`), **Roles** (`/administration/users/roles`), **Account security** (`/administration/users/account-security`) and **Privacy Requests** (`/administration/users/privacy-requests`) pages with shared tab navigation and route-derived breadcrumbs. The Users page contains a directory summary, status tabs, debounced search, role filtering, compact identity, role and status columns, and server-backed page-size controls. The Invitations page contains the invitation lifecycle card with status counts and page-size controls. **Invite user** opens the same shared form from either page. Filters remain in the URL so directory and invitation views can be revisited or shared. Open a person's name to inspect their access, assign roles or change their account status.
