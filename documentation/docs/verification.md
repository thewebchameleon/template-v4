...
`node --test tools/auth-retry.test.mjs` runs isolated Node/RxJS regression checks against the actual interceptor and route-reuse source. It launches no browser. `ReviewHardeningTests` covers organization S3 storage, file capabilities, expired billing cancellation, organization onboarding/closure, privacy cache redaction, retention failures, passkey policy and API feature gating against PostgreSQL. Browser/E2E and merchant sandbox verification remain separate.
