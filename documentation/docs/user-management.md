...
The Users workspace exposes bookmarkable Users, Invitations, Roles, Account security and Privacy Requests pages at `/administration/users`, `/administration/users/invitations`, `/administration/users/roles`, `/administration/users/account-security` and `/administration/users/privacy-requests`. They share tab navigation and each child page updates the breadcrumb. The Users directory combines Identity username and display name in a compact identity cell, with email, roles and status in separate server-sortable columns. Search matches username, display name or email. Identity username is returned independently of email; existing email-based accounts may have the same value in both fields. Status tabs expose Active, Invited and Disabled counts for the current search and role scope. Role and status filters, page size, sorting and search persist in URL query state; changing any filter resets pagination to page 1. The role filter opens in the shared right-side drawer, whose clear action is available only while a directory filter is applied. The Invitations page keeps its own namespaced URL-backed filters, page size and pagination.

Click a user row, or activate its username button with the keyboard, to open the complete user-details editor in a right drawer. The directory URL and query state remain unchanged. The drawer reuses the routed details component, retains permission and concurrency checks, confirms discarding unsaved changes, and refreshes the directory after a save. Direct `/administration/users/{id}` links continue to show the standalone page.
...
Privacy Requests is available beside Account security at `/administration/users/privacy-requests` with the same `settings.manage` permission as before. The previous `/administration/privacy-requests` and `/privacy-requests` routes are removed without redirects.
