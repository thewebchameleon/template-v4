...
Use DocMD as an independent root-level workspace in `documentation` to validate, render, and serve the Markdown sources in `documentation/docs`. Pin the DocMD version in both the framework manifest and the npm lockfile. Aspire starts the DocMD development server; CI builds the static site.
