...
- Accept arbitrary file bytes and extensions, including empty files, with an administrator-configurable upload read bound: 5 MiB increments through 100 MiB, 10 MiB increments through 200 MiB, 50 MiB increments through 500 MiB, 100 MiB increments through 1,000 MiB, then unlimited. The default is 20 MiB. Stage request bodies in delete-on-close temporary files rather than application memory, apply the setting to personal and organisation files, advertise it to clients, and disable the reverse-proxy body limit so the application setting remains authoritative. Keep attachment disposition, octet-stream content type, nosniff and private object storage. This does not provide previews or malware scanning. Scan/quarantine before marking a reservation ready if adding scanning later.
- Store file metadata and the versioned default quota in the File Storage module's PostgreSQL schema. The default is initially 100 MiB. Administrators select it under File Storage using a stepped slider starting at 50 MiB: 5 MiB increments through 100 MiB, 10 MiB through 200 MiB, 50 MiB through 500 MiB, 100 MiB through 5,000 MiB, 1,000 MiB through 20,000 MiB, then unlimited. `-1` persists the final unlimited position. A nullable override remains on each Identity user because account administration owns that setting. Null follows the current default for existing and future users, while individual overrides remain numeric from zero through 100 GiB; an individual zero override blocks uploads. UI MB units retain the existing binary convention (1 MB = 1,048,576 bytes). The old configuration-only `Storage:QuotaBytes` setting is superseded.
