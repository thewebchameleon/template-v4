...
- Store one versioned default quota in PostgreSQL, initially 100 MiB, and a nullable override on each Identity user. Null follows the current default for existing and future users. Administrators edit the default under File storage, or the override through user details. Bounds are zero through 100 GiB; zero blocks uploads. UI MB units retain the existing binary convention (1 MB = 1,048,576 bytes). The old configuration-only `Storage:QuotaBytes` setting is superseded.
