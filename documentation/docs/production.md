...
The platform baseline now requires a private HTTPS S3 endpoint and app credentials in production. Follow [object storage](object-storage.md) before deploying API/Worker, and apply the PlatformBaseline migration through the migrator. Add [monitoring](monitoring.md) to configure alerts and repeatable restore verification. System Health identifies the release from assembly metadata stamped during the build.
...
Set every value listed in `deploy/compose-platforms/.env.example` in the deployment environment. PostgreSQL and the Migrator, API and Worker roles share `POSTGRES_PASSWORD`; Compose builds their Npgsql connection strings. Do not point this configuration at a development database volume. For an existing database, changing the value requires deliberately rotating all four role passwords because initialization scripts do not rerun.

Supply a 3072-bit or stronger base64-encoded RSA PEM signing key and SMTP credentials for a real delivery provider. The deployment environment is not an encrypted vault; restrict platform and Docker access. The persistent Data Protection key volume is explicitly allowed to remain unencrypted for this Compose deployment and must be protected and backed up. Mailpit is not part of the production manifest.
