...
Supply a 3072-bit or stronger RSA PEM signing key, a password-protected PKCS#12 Data Protection wrapping certificate, its password, and SMTP credentials for a real delivery provider. Secret files need permissions readable by the intended .NET container UID (1654) and protected against other host users. Compose file-backed secrets are host files, not an encrypted secret vault. The image supplies a mode-0700 `/keys/ring` directory inside the persistent key volume. Mailpit is not part of the production manifest.
