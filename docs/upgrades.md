...
Application now references Framework.Core. Keep the new project when copying the repository, or use its matching NuGet package. Add custom domain-event handlers and integration-contract registration explicitly. Generated email/localisation scaffolds are runtime-connected. CLI doctor checks Node/.NET versions; upgrade remains an explicit informational command, not an automatic migration engine.
