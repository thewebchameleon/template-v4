...
- Follow the root minimum-test policy: reuse existing coverage and add only the fewest cases needed for changed behavior or a regression. Test at the layer that demonstrates the behavior; touching multiple layers does not require new tests in each. Preserve required security, persistence, session, and messaging checks without adding speculative cases or elaborate scaffolding.
- Choose focused checks from current package scripts and stop once the required checks pass unless a concrete concern remains. Surface missing prerequisites and remaining validation; do not bypass E2E permission.
